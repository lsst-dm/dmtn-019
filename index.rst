...
These simulated images contain a realistic distribution of different stellar types, and using several thousand different model SEDs (interpolated Kurucz models, from sims_photUtils).
...
I ran two full simulation runs, each with a different random distribution of stars. 
...
Figures 1 and 2 below show a small sub-region of the "Faint" and "Bright" difference images for each band at an airmass of 1.3, along with the g-band template image for reference. See :ref:`section-label-additional` for images of the full fields.
...
* Next, calculate the total significance of both positive and negative sources in the difference image, and divide by the uncertainty in their flux.
